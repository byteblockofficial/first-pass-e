...
export default function ContactForm() {
  ...
  if (status === "success") return (
    <div role="alert" className="bg-[var(--color-surface)] border border-[var(--color-border)] rounded-lg p-8 text-center">
      ...
      <p className="text-sm text-[var(--color-text-muted)]">We&apos;ll be in touch shortly. Or call <a href="tel:01610000007" className="underline">0161 000 0007</a>.</p>
    </div>
  );

  return (
    <form onSubmit={handleSubmit} noValidate className="space-y-4">
      {status === "error" && <div role="alert" className="bg-red-50 border border-red-200 text-red-800 text-sm px-4 py-3 rounded-md">Something went wrong. Please call <a href="tel:01610000007" className="underline">0161 000 0007</a>.</div>}
      <div>
      ...
      <button type="submit" disabled={status === "loading"} className="w-full bg-[var(--color-secondary)] hover:bg-[var(--color-secondary-dark)] disabled:opacity-60 text-white font-semibold py-3 rounded-md transition-colors">
