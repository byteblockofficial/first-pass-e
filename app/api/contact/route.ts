...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    await resend.emails.send({ from: "noreply@first-pass-driving.vercel.app", to: recipientEmail, replyTo: rawEmail || undefined, subject: `New enquiry from ${name}`, text: [`Name:    ${name}`, `Email:   ${email}`, `Phone:   ${phone}`, ``, `Message:`, message].join("\n") });
